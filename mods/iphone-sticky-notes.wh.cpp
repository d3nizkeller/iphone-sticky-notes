...
// @description     Desktop sticky notes with an iPhone-style glass look, editable text, colors, drag, autosave, and a small add-note button.
// @version         1.0
...
Features:
- iPhone-inspired rounded cards and translucent floating add button.
...
void ApplyEditColor(NoteWindow* note) {
    ...
    note->brush = CreateSolidBrush(note->color);
...
LRESULT CALLBACK NoteProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CTLCOLOREDIT:
        SetBkColor(reinterpret_cast<HDC>(wParam), note->color);
    ...
    case WM_PAINT: {
        ...
        HBRUSH body = CreateSolidBrush(note->color);
        HBRUSH header = CreateSolidBrush(RGB(255, 255, 255));
        RECT rc{0, 0, kNoteWidth, kNoteHeight};
        FillRect(hdc, &rc, body);
        RECT head{0, 0, kNoteWidth, kHeaderHeight};
        FillRect(hdc, &head, header);
        SetBkMode(hdc, TRANSPARENT);
        SetTextColor(hdc, RGB(70, 70, 76));
        RECT title{14, 0, kNoteWidth - 74, kHeaderHeight};
        DrawTextW(hdc, L"iPhone Note", -1, &title, DT_VCENTER | DT_SINGLELINE);
...
LRESULT CALLBACK LauncherProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT: {
        PAINTSTRUCT ps;
        HDC hdc = BeginPaint(hwnd, &ps);
        HBRUSH brush = CreateSolidBrush(RGB(0, 122, 255));
        RECT rc{0, 0, 54, 54};
...
DWORD WINAPI UiThread(LPVOID) {
    ...
    SetLayeredWindowAttributes(g_launcher, 0, 230, LWA_ALPHA);
    LoadNotes();
